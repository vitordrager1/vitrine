...
import { SimpleGrid, Spinner, Flex, IconButton, Heading } from "@chakra-ui/react";
...
const Informatica = () => {
    ...
    // }
      const getproduto = useCallback(async (paramOffset) => {
        try {
            setLoading(true)
            const { data } = await axios.get('http://localhost/admin/api/categoria/1', {
                    params: {
                    limit: 20,
                    offset: paramOffset
                }
  ...
  return (
    ...
    <div className="container">
        ...
        <Navbar/>
        <Heading p={"5"}>Informática</Heading>
        {renderProdutoList()}
    </div>
